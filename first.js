...
function openModal(carId) {
    ...
    const user     = getCurrentUser();
    const bookings = JSON.parse(localStorage.getItem('edelhausTestDrives')) || [];
    const booking  = user && bookings.find(b => b.carId === carId && b.userEmail === user.email);

    // Build booking confirmation block (shows on reload if already booked)
    let bookingBlock = '';
    if (booking) {
        bookingBlock = `
        <div class="booking-status-card mt-3">
            <div class="booking-status-header">
                <i class="bi bi-calendar-check-fill text-success me-2"></i>
                <strong>Test Drive Confirmed</strong>
                <span class="ms-auto badge bg-success" style="font-size:0.7rem;">SAVED</span>
            </div>
            <div class="booking-status-body">
                <div class="booking-detail-row"><span class="booking-detail-label"><i class="bi bi-person me-2"></i>Name</span><span class="booking-detail-value">${booking.name}</span></div>
                <div class="booking-detail-row"><span class="booking-detail-label"><i class="bi bi-envelope me-2"></i>Email</span><span class="booking-detail-value">${booking.email}</span></div>
                <div class="booking-detail-row"><span class="booking-detail-label"><i class="bi bi-telephone me-2"></i>Phone</span><span class="booking-detail-value">${booking.phone}</span></div>
                <div class="booking-detail-row"><span class="booking-detail-label"><i class="bi bi-calendar3 me-2"></i>Date</span><span class="booking-detail-value">${formatDateDisplay(booking.date)}</span></div>
                <div class="booking-detail-row"><span class="booking-detail-label"><i class="bi bi-clock me-2"></i>Time</span><span class="booking-detail-value">${booking.time}</span></div>
                ${booking.notes ? `<div class="booking-detail-row"><span class="booking-detail-label"><i class="bi bi-chat-text me-2"></i>Notes</span><span class="booking-detail-value">${booking.notes}</span></div>` : ''}
            </div>
            <button class="btn btn-sm btn-outline-danger mt-2 w-100" onclick="cancelBooking(${carId})">
                <i class="bi bi-x-circle me-1"></i>Cancel Booking
            </button>
        </div>`;
    }

    let allImages = car.images ? [...car.images] : [];
    if (car.colors) car.colors.forEach(c => { if (!allImages.includes(c.img)) allImages.push(c.img); });

    const thumbsHTML = allImages.length > 1 ? `
        <div class="d-flex gap-2 mt-2 flex-wrap">
            ${allImages.map((img, i) => `<img src="${img}" onclick="swapModalImg('${img}',this)" class="modal-thumb ${i === 0 ? 'active-thumb' : ''}" alt="img">`).join('')}
        </div>` : '';

    document.getElementById('carBody').innerHTML = `
        <div class="position-relative mb-2">
            <img id="modalMainImg" src="${allImages[0] || ''}" class="img-fluid rounded w-100" style="max-height:300px;object-fit:cover;transition:opacity 0.3s;">
            <button class="btn btn-sm btn-dark position-absolute top-0 end-0 m-2" onclick="openGalleryModal(${car.id})" style="opacity:0.85;border-radius:8px;">
                <i class="bi bi-fullscreen me-1"></i>Gallery
            </button>
        </div>
        ${thumbsHTML}
        <h4 class="mt-3">${formatPrice(car.price)}</h4>
        <div class="row mt-3">
            <div class="col-6"><p><strong>Engine:</strong> ${car.engine || 'N/A'}</p></div>
            ...
            <div class="col-6"><p><strong>Category:</strong> <span style="text-transform:capitalize">${car.category}</span></p></div>
            <div class="col-6"><p><strong>Status:</strong> <span class="text-warning">${car.status}</span></p></div>
        </div>
        <p class="mt-2 text-muted">${car.description}</p>
        ${bookingBlock}
        <div class="d-flex gap-2 mt-3">
            <button class="btn btn-gradient flex-grow-1" onclick="requestTestDrive(${car.id})">
                <i class="bi bi-calendar-check me-2"></i>${booking ? 'Modify Booking' : 'Book Test Drive'}
            </button>
            <button class="btn btn-outline-light px-3" onclick="downloadBrochure(${car.id})" title="PDF Brochure">
...
function renderFeaturedCars() {
    ...
    if (!g) return;
    g.innerHTML = carsData.filter(c => c.featured).map((c, i) => generateCarCard(c, i)).join('');
...
// ==============================================================
let invCards      = [];   // full deduplicated car list
let invIndex      = 0;    // index of leftmost visible card
let invAutoTimer  = null;
let invAnimating  = false;
const INV_VISIBLE  = 3;   // cards visible at once
const INV_INTERVAL = 3000; // ms between auto-slides
...
function renderInventoryCars() {
    ...
    const seen = new Set();
    invCards = filtered.filter(c => { if (seen.has(c.id)) return false; seen.add(c.id); return true; });

    if (invCards.length === 0) {
        track.innerHTML = '';
        if (noRes) noRes.style.display = 'block';
        if (wrapper) { wrapper.querySelector('.inv-arrow-prev').style.display = 'none'; wrapper.querySelector('.inv-arrow-next').style.display = 'none'; }
        return;
    }
    ...
    const showArrows = invCards.length > INV_VISIBLE;
    if (wrapper) {
        wrapper.querySelector('.inv-arrow-prev').style.display = showArrows ? 'flex' : 'none';
        wrapper.querySelector('.inv-arrow-next').style.display = showArrows ? 'flex' : 'none';
    }
...
// Build the track with clones at start & end for seamless looping
function invBuildTrack(track) {
    if (invCards.length <= INV_VISIBLE) {
        track.innerHTML = invCards.map((car, i) => `
            <div class="inv-card-slot">
                <div class="row g-0">
                    ${generateCarCard(car, i)}
                </div>
            </div>`).join('');
    ...
    const clonesBefore = invCards.slice(-INV_VISIBLE);
    const clonesAfter  = invCards.slice(0, INV_VISIBLE);
    const all          = [...clonesBefore, ...invCards, ...clonesAfter];

    track.innerHTML = all.map((car, i) => `
        <div class="inv-card-slot">
            <div class="row g-0">
                ${generateCarCard(car, i % INV_VISIBLE)}
            </div>
        </div>`).join('');

    track.style.transition = 'none';
    invSetPosition(track, INV_VISIBLE, false);
}

function invSetPosition(track, idx, animate) {
    if (!track) return;
    const cardW = 100 / INV_VISIBLE; // each card = 33.33% of track width
...
function invMove(dir) {
    const track = document.getElementById('inventoryGrid');
    if (!track || invAnimating || invCards.length <= INV_VISIBLE) return;

    invStopAuto();
    invAnimating = true;

    // The real cards start at offset INV_VISIBLE in the DOM
    const domIdx = INV_VISIBLE + invIndex + dir;
    invSetPosition(track, domIdx, true);

    track.addEventListener('transitionend', function handler() {
        track.removeEventListener('transitionend', handler);

        // Update real index with wrap-around
        invIndex = ((invIndex + dir) % invCards.length + invCards.length) % invCards.length;

        // Silently jump to real position (no animation)
        invSetPosition(track, INV_VISIBLE + invIndex, false);

        invAnimating = false;
    });

    if (dir > 0) invIndex; // update after transition
    invStartAuto();
}
...
function filterCars(cat) {
    currentBrandFilter = cat; invIndex = 0;
